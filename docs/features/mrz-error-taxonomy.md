...
The three capture-availability members landed with the owns-the-camera-session layer (`MrzCameraScanner`). They are surfaced — never thrown — on the scanner's `results` stream as a `MrzScanResult.CaptureError` when the camera cannot start. Host tests produce each member through the scanner's surfacing path and assert its stable code. The Android scanner's failure-to-member mapping is still coarse in this slice: it maps a permission failure to `PermissionDenied` and any other camera-open failure to `CameraUnavailable`. Distinguishing `CameraInUse` — which CameraX surfaces asynchronously through camera state rather than as a bind-time exception — is completed and verified on a device in the live-device slice, where the full mapping is shaken out.
