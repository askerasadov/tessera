...
| Layer 3 — Country code recognition (`MrzUnknownCountryCode`) | Implemented (TD3; surfaces as a warning per [ADR-013](../decisions/0013-recognition-failures-are-warnings.md), with the deliberately-incomplete starter set tracked in `docs/open-questions.md` "Country code table completeness") |
| Layer 3 — Document type code recognition (`MrzUnknownDocumentTypeCode`) | Implemented (TD3; surfaces as a warning per [ADR-013](../decisions/0013-recognition-failures-are-warnings.md), with the deliberately-incomplete starter set tracked in `docs/open-questions.md` "Document type code table completeness") |
...
| Name truncation warning (`MrzNameTruncated`) | Implemented (TD3; signal-driven from `commonFields.nameTruncated`, which the parser populates per ICAO Doc 9303 convention — see `mrz-parsing.md` "Truncated Names") |
