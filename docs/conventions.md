...
Adding a new MRZ format (beyond the ICAO Doc 9303 formats already supported) requires coordinated changes across several parts of the codebase and documentation. The following must be done together:

1. Define the format specification in the shared `formats/` package within `mrz-core` — field positions, field widths, check digit positions, padding rules, character set rules
2. Add a new variant to the `MrzDocument` sealed hierarchy reflecting the format's fields
3. Implement the parsing path for the format
4. Implement the generation path for the format
5. Implement the validation path for the format
6. Add the format to the auto-detect logic in the parser, with structural cues that distinguish it from existing formats
7. Update `scope.md` to list the format as supported
8. Update or extend the relevant feature documents (`mrz-data-model.md`, `mrz-parsing.md`, `mrz-generation.md`, `mrz-validation.md`) to describe the format
9. Add synthetic test fixtures generated by the format's own generator path, covering at minimum the canonical example from the format's source specification
10. Update the changelog to note the addition

A format is not "added" until all of the above are complete. Partial additions (a format that parses but does not generate, or that lacks documentation) are not acceptable in a release.

The same procedure applies in spirit to non-MRZ formats added in the future (chip data formats, document image formats, etc.), with steps adjusted for the relevant subsystems.
