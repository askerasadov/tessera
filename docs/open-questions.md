...
The iOS scanner (`AVCaptureMrzScanner`) now adds a second dimension to the same question. It maps an `AVCaptureSession` interruption with reason `videoDeviceInUseByAnotherClient` to `CameraInUse`, and a session runtime error to `CameraUnavailable`; permission is read from `AVCaptureDevice.authorizationStatus(for:)`. None of this — the in-use interruption, the runtime-error path, ordinary live streaming, or the `CMSampleBuffer` retain/release accounting under frame load — is exercised on the iOS Simulator (it has no camera), so the whole iOS scanner is **compile-verified on CI and device-verified separately**, exactly as the Android scanner shipped (CameraX wiring compiled on CI, then device-verified). The iOS `CameraInUse` interruption shares the Android `CameraInUse` gap: reproducing it needs a second client holding the camera. The iOS interruption is also a candidate for the same terminal-vs-transient design point — the scanner currently treats only the in-use reason as terminal and leaves backgrounded / system-pressure interruptions for AVFoundation to recover.

**Status:** Open — on Android, the recoverable/critical split is confirmed from the androidx source; what remains is the in-use *behaviour* on a device and the terminal-vs-recoverable contract for recoverable codes (the critical/permission path is device-verified and correct). On iOS, the scanner's whole live path (streaming, the async in-use / runtime-error mapping, and `CMSampleBuffer` lifetime under load) is compile-verified and awaits a device run.
