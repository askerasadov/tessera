...
**Device-verified (2026-05-31, iPhone 15 Pro / iOS 26.5, throwaway SwiftUI harness on the `Tessera` XCFramework):** the clean shipping build streamed **770 analysed frames with zero freezes and zero dropped frames**, memory a bounded sawtooth **96–253 MB** (auto-GC reclaiming on its rhythm), and **live `Decoded` from a real document — 34 `ParseResult.Success` + 86 `PartialSuccess`**. This also closes the previously-open **live `Decoded` lock** gap. The pitfall (Cocoa weak delegate + K/N GC) is recorded in [`.claude/known-pitfalls.md`](../.claude/known-pitfalls.md). Still unverified on iOS: the `CameraUnavailable` runtime-error path.

**Status:** Partially open. **iOS capture stall: RESOLVED** (above); iOS live `Decoded`: **device-verified**. Still open — Android: recoverable/critical split confirmed from androidx source, but in-use *behaviour* and the terminal-vs-recoverable contract await a second-client device test; iOS: the `CameraUnavailable` runtime-error path is not yet device-exercised.

**Source:** 2026-05-30 Android live-device slice; 2026-05-31 iOS device run, stall root-cause + fix, and live-decode verification.

**Trigger:** A device test with a second camera client (Android in-use behaviour) or the first real consumer hitting camera contention; revisit the terminal-vs-recoverable contract then. The iOS `CameraUnavailable` runtime-error path when a future device scenario can trigger it.
