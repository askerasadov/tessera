...
**Status:** Partially open. **iOS capture stall: RESOLVED**; iOS live `Decoded`: **device-verified**; iOS capture-error → `CameraError` mapping: **unit-test-locked**, with the `CameraUnavailable`-from-runtime-error trigger accepted as not deliberately reproducible (Apple docs). Still open — Android: recoverable/critical split confirmed from androidx source, but in-use *behaviour* and the terminal-vs-recoverable contract await a second-client device test.

**Source:** 2026-05-30 Android live-device slice; 2026-05-31 iOS device run, stall root-cause + fix, and live-decode verification.

**Trigger:** A device test with a second camera client (Android in-use behaviour) or the first real consumer hitting camera contention; revisit the terminal-vs-recoverable contract then. The iOS `CameraUnavailable` runtime-error path would only be observable live via an organic media-services/hardware fault or a consumer report — not deliberately reproducible, so no proactive test is planned.
