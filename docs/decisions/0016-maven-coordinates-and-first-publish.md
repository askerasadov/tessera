...
- **iOS distribution channel.** CocoaPods vs Swift Package Manager (SPM) for the iOS target remains deferred under the "Distribution channels" entry in [`open-questions.md`](../open-questions.md). Tied to iOS target activation per [`scope.md`](../scope.md), which awaits Xcode availability.
